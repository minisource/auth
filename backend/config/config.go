...
type WebAuthnConfig struct {
	Enabled    bool
	RPID       string   // Relying Party ID — the effective domain (no scheme/port)
	RPName     string   // Display name shown in the browser prompt
	RPOrigins  []string // Allowed origins (full https:// origins the browser may call from)
	ChallengeTTL string // Challenge expiry (e.g. "5m")
}
...
func GetConfig() *Config {
	once.Do(func() {
		...
		cfg = &Config{
			...
			WebAuthn: WebAuthnConfig{
				Enabled:      getEnvAsBool("WEBAUTHN_ENABLED", true),
				RPID:         getEnv("WEBAUTHN_RP_ID", "localhost"),
				RPName:       getEnv("WEBAUTHN_RP_NAME", "DiviPay"),
				RPOrigins:    splitCSV(getEnv("WEBAUTHN_RP_ORIGINS", "http://localhost:3000,http://localhost:3001")),
				ChallengeTTL: getEnv("WEBAUTHN_CHALLENGE_TTL", "5m"),
			},
		}
	})
