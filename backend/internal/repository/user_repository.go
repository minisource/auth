...
type UserRepository interface {
	...
	ListWithFilters(ctx context.Context, tenantID *uuid.UUID, search string, roleID uuid.UUID, isActive *bool, offset, limit int) ([]models.User, int64, error)
...
func (r *userRepository) ListWithFilters(ctx context.Context, tenantID *uuid.UUID, search string, roleID uuid.UUID, isActive *bool, offset, limit int) ([]models.User, int64, error) {
	var users []models.User
	var total int64
	...
	if tenantID != nil && *tenantID != uuid.Nil {
		query = query.Where("(users.tenant_id = ? OR users.id IN (SELECT user_id FROM tenant_members WHERE tenant_id = ?))", *tenantID, *tenantID)
	}
