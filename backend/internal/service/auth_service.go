...
func (s *AuthService) Login(ctx context.Context, req *LoginRequest) (*AuthResponse, error) {
	...
	if user.TwoFactorEnabled {
		if req.TwoFactorCode == "" {
			twoFactorToken, _, err := s.tokenService.GenerateTwoFactorToken(user.ID)
			if err != nil {
				return nil, err
			}
			return &AuthResponse{
		...
		if !ValidateTOTPCode(user.TwoFactorSecret, req.TwoFactorCode) {
			s.logLoginAttempt(ctx, user.ID, uuid.Nil, models.LoginActionLoginFailed, req.IPAddress, req.UserAgent, false, "invalid 2fa code")
...
func (s *AuthService) VerifyOTPAndLogin(ctx context.Context, target, code, otpType, twoFactorCode, ipAddress, userAgent string) (*AuthResponse, error) {
	...
	if user.TwoFactorEnabled {
		...
		if !ValidateTOTPCode(user.TwoFactorSecret, twoFactorCode) {
			s.logLoginAttempt(ctx, user.ID, uuid.Nil, models.LoginActionLoginFailed, ipAddress, userAgent, false, "invalid 2fa code")
			return nil, NewTwoFactorInvalidError()
		}
	}
...
// VerifyTwoFactorAndLogin completes the 2FA login step using a challenge token + TOTP code.
func (s *AuthService) VerifyTwoFactorAndLogin(ctx context.Context, twoFactorToken, code, ipAddress, userAgent string) (*AuthResponse, error) {
	claims, err := s.tokenService.ValidateTwoFactorToken(twoFactorToken)
	if err != nil {
...
// CreatePasskeySession creates a full session after a successful WebAuthn
// passkey assertion and records it as a passkey login in the audit log.
func (s *AuthService) CreatePasskeySession(ctx context.Context, user *models.User, ipAddress, userAgent string) (*AuthResponse, error) {
	return s.createAuthSessionWithAction(ctx, user, ipAddress, userAgent, models.LoginActionPasskeyLogin)
}

// Helper methods

func (s *AuthService) createAuthSession(ctx context.Context, user *models.User, ipAddress, userAgent string) (*AuthResponse, error) {
	return s.createAuthSessionWithAction(ctx, user, ipAddress, userAgent, models.LoginActionLogin)
}

func (s *AuthService) createAuthSessionWithAction(ctx context.Context, user *models.User, ipAddress, userAgent, action string) (*AuthResponse, error) {
	// Create session
	...
	s.logLoginAttempt(ctx, user.ID, session.ID, action, ipAddress, userAgent, true, "")
...
func (s *AuthService) GetSeedStatus(ctx context.Context) *SeedStatusResponse {
	...
	if sysadminPassword == "" {
		sysadminPassword = "changeme123"
	}

	user, err := s.userRepo.GetByEmail(ctx, sysadminEmail)
	if err != nil || user == nil {
	...
	// Verify if the stored password hash still matches the default password
	isDefaultPassword := s.passwordService.VerifyPassword(sysadminPassword, user.PasswordHash)
	...
	if isDefaultPassword {
		return &SeedStatusResponse{
			...
			Email:              sysadminEmail,
			DefaultPassword:    sysadminPassword,
		}
	}

	// Admin has changed their password; hide notice everywhere across all sessions/browsers
	return &SeedStatusResponse{
		ShowNotice:         false,
		HasDefaultPassword: false,
	}
