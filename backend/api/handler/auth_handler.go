...
func handleAuthError(c *fiber.Ctx, err error, logger logging.Logger) error {
	...
	if errors.As(err, &serviceErr) {
		...
		case service.ErrCodeInvalidCredentials, service.ErrCodeOTPInvalid, service.ErrCodeOTPExpired, service.ErrCodeRefreshTokenInvalid, service.ErrCodeTwoFactorInvalid, service.ErrCodeWebAuthnChallenge:
		...
		case service.ErrCodeEmailExists, service.ErrCodePhoneExists, service.ErrCodeUsernameExists, service.ErrCodePasskeyExists:
			return response.Conflict(c, message)
		case service.ErrCodeUserNotFound, service.ErrCodeRoleNotFound, service.ErrCodePermissionNotFound, service.ErrCodePasskeyNotFound, service.ErrCodeNoPasskeys:
...
// @Accept json
// @Produce json
// @Param request body dto.VerifyTwoFactorRequest true "2FA verification"
// @Success 200 {object} dto.AuthResponse
// @Failure 401 {object} dto.ErrorResponse
...
func (h *AuthHandler) VerifyTwoFactor(c *fiber.Ctx) error {
	...
	resp, err := h.authService.VerifyTwoFactorAndLogin(c.Context(), req.TwoFactorToken, req.Code, c.IP(), c.Get("User-Agent"))
	if err != nil {
		return handleAuthError(c, err, h.logger)
	}
...
// GetSeedStatus godoc
// @Summary Seed credentials status
// @Description Check if default seed credentials are valid and active
// @Tags Auth
// @Produce json
// @Success 200 {object} response.Response
// @Router /auth/seed-status [get]
