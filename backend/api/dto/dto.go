...
// PasskeyLoginBeginRequest begins a passkey login for the account with this email.
type PasskeyLoginBeginRequest struct {
	Email string `json:"email" validate:"required,email"`
}

// PasskeyRegistrationFinishRequest names the passkey and carries the raw
// authenticator response (the browser's PublicKeyCredential JSON).
type PasskeyRegistrationFinishRequest struct {
	Name       string          `json:"name,omitempty"`
	Credential json.RawMessage `json:"credential" validate:"required"`
}

// PasskeyLoginFinishRequest carries the raw assertion response.
